/**
 * Sender-facing reply copy for the intake pipeline. Channel-neutral string
 * builders — the adapter decides how to deliver them.
 *
 * Builders that a landlord sees on the critical path take an optional trailing
 * `lang` and consult `./i18n` first, falling back to the English literal right
 * here. That English literal is still the code that runs for every English
 * landlord, unchanged — the translation is purely additive, and a missing key
 * or a disabled `enableLocalizedReplies` flag lands back on it.
 *
 * These are therefore no longer pure in the strictest sense: `t()` reads the
 * synchronous feature-flag snapshot. It stays deterministic per request, and
 * tests set the flag explicitly.
 */
...
const label = (f: string, lang: ReplyLang = 'en') =>
  t(lang, `field.${f}` as Parameters<typeof t>[1]) ?? FIELD_LABELS[f] ?? f;

/** "a, b and c" — a field ask should read like a sentence, not a CSV row. */
const naturalJoin = (items: string[], lang: ReplyLang = 'en'): string => {
  if (items.length <= 1) return items[0] ?? '';
  const and = t(lang, 'listJoin') ?? 'and';
...
export function summarizeUnderstood(parsed: {
  ...
  if (lang !== 'en') {
    // Built as a dot-separated list rather than a translated sentence. Sinhala
    // and Tamil put the noun, the place and the price in a different order from
    // English, and each part inflects — a fill-in-the-blanks sentence would come
    // out subtly wrong in a way an English speaker could not review. A list is
    // unambiguous in every language and still tells the landlord exactly what we
    // read from their message.
    const parts: string[] = [];
    if (parsed.bedrooms) {
      parts.push(t(lang, 'understood.bedrooms', { n: parsed.bedrooms }) ?? `${parsed.bedrooms} bedrooms`);
    }
    const typeKey = type ? (`type.${type}` as Parameters<typeof t>[1]) : null;
    const typeWord = typeKey ? t(lang, typeKey) : null;
    if (typeWord) parts.push(typeWord);
    if (parsed.city) parts.push(t(lang, 'understood.city', { city: parsed.city }) ?? parsed.city);
    if (parsed.rentPerMonth) {
      parts.push(
        t(lang, 'understood.rent', { rent: parsed.rentPerMonth.toLocaleString('en-US') }) ??
          `LKR ${parsed.rentPerMonth.toLocaleString('en-US')}`
      );
    }
    if (parts.length) return parts.join(' · ');
    // Nothing translatable came out — fall through to English rather than
    // returning an empty echo, which would read as "the bot ignored me".
  }
...
export function needsInfoMessage(
  ...
  opts: {
    ...
    /** Set when the ad reads as a property FOR SALE — see parser/sale-ad.ts. */
    looksLikeSaleAd?: boolean;
  } = {},
  lang: ReplyLang = 'en'
): string {
  ...
  // A for-sale ad has no monthly rent to give us, so asking for one loops
  // forever. Say what is actually wrong instead, and leave the door open.
  if (opts.looksLikeSaleAd) {
    return (
      t(lang, 'saleAd', { name }) ??
      `${greeting} This looks like a property for sale. Easy Rent is for rentals only — we don't publish properties for sale.\n\nIf you'd like to rent it out instead, send us the monthly rent and we'll create the listing.`
    );
  }
  ...
  const echo = opts.understood
    ? (t(lang, 'needsInfoEcho', { summary: opts.understood }) ?? ` Got it — ${opts.understood}.`)
    : '';
  ...
  const fields = naturalJoin(
    missingFields.map((f) => label(f, lang)),
    lang
  );
  const base = missingFields.length
    ? (t(lang, 'needsInfo', { name, echo, fields }) ??
      `${greeting}${echo} To publish we still need: ${fields}. Just reply here with the details.`)
    : (t(lang, 'needsInfoNoFields', { name }) ??
 ...
 */
export function receivedAckMessage(profileName: string | null, lang: ReplyLang = 'en'): string {
  return (
    t(lang, 'receivedAck', { name: profileName ? ', ' + profileName : '' }) ??
    `Got it${profileName ? ', ' + profileName : ''}! We're putting your listing together — you'll hear from us here in a few minutes.`
  );
}
...
export function updateAckMessage(lang: ReplyLang = 'en'): string {
  return t(lang, 'updateAck') ?? `Thanks — got it! Updating your listing now, give us a few minutes.`;
}
 ...
 */
export function manualReviewMessage(profileName: string | null, lang: ReplyLang = 'en'): string {
  return (
    t(lang, 'manualReview', { name: profileName ? ' ' + profileName : '' }) ??
    `Thanks${profileName ? ' ' + profileName : ''}! Our team is taking a quick look at your listing and will get back to you here soon.`
  );
}
...
export function photosMissedMessage(failed: number, lang: ReplyLang = 'en'): string {
  return (
    t(lang, 'photosMissed', { failed }) ??
    `⚠️ ${failed} photo${failed === 1 ? '' : 's'} didn't come through — please send ${failed === 1 ? 'it' : 'them'} again.`
  );
}
...
export function publishedMessage(
  ...
  const parts = [
    t(lang, 'published.live', { title }) ?? `🎉 Your listing "${title}" is now LIVE on Easy Rent:`,
    links.viewUrl,
  ];
  if (links.editUrl) {
    parts.push(
      '',
      t(lang, 'published.edit') ?? '✏️ Edit it yourself (opens instantly, no password):',
      links.editUrl
    );
  }
  if (links.deleteUrl) {
    parts.push('', t(lang, 'published.remove') ?? '🗑️ Remove it:', links.deleteUrl);
  }
  ...
  parts.push(
    '',
    t(lang, 'published.photos') ?? '📷 Want to add photos? Just send them here within 2 days.'
  );
...
export function deleteMenuMessage(items: DeleteMenuItem[], lang: ReplyLang = 'en'): string {
  const lines = [t(lang, 'delete.which') ?? 'Which listing would you like to remove?', ''];
  for (const i of items) {
    const where = i.city ? `, ${i.city}` : '';
    const pending = i.status === 'pending' ? ' (awaiting review)' : '';
    lines.push(`${i.index}. ${i.title}${where}${pending}`);
  }
  lines.push('', t(lang, 'delete.replyNumber') ?? 'Reply with the number, or CANCEL to stop.');
  return lines.join('\n');
}

/** Step 2 of delete: an exact typed word. Anything else cancels. */
export function deleteConfirmMessage(title: string, lang: ReplyLang = 'en'): string {
  return (
    t(lang, 'delete.confirm', { title }) ??
    `You're about to remove "${title}" from Easy Rent.\n\nReply DELETE to confirm, or anything else to cancel.`
  );
}

export function deleteDoneMessage(title: string, lang: ReplyLang = 'en'): string {
  return (
    t(lang, 'delete.done', { title }) ??
    `🗑️ Removed "${title}". It's no longer visible to tenants.\n\nChanged your mind? Reply RESTORE within 30 days and we'll put it back. Or send new details any time to list again.`
  );
}
...
export function deleteCancelledMessage(lang: ReplyLang = 'en'): string {
  return (
    t(lang, 'delete.cancelled') ??
    `No changes made. Send DELETE again if you'd like to remove a listing.`
  );
}
...
/** Reply to "HELP" / "MENU". */
export function helpMessage(lang: ReplyLang = 'en'): string {
  return t(lang, 'help') ?? [
 ...
 * or a public TikTok.
 */
export function socialConsentPrompt(title: string, lang: ReplyLang = 'en'): string {
  return t(lang, 'socialConsent', { title }) ?? [
